/* =========================================================
   BG·LAB — интерфейс: навигация, списки, панели, экспорт
   ========================================================= */

/* ---------- 8. Состояние и навигация ---------- */
var state=null, sel=0, bgSel=0, raf=0;
var stage=$('stage'), listEl=$('list'), lookEl=$('lookCtrls'), animEl=$('animCtrls'),
    lookTitle=$('lookTitle'), animSec=$('animSec'), cardsEl=$('cards');

function openEditor(p){
  state=p.st(); sel=0; bgSel=0;
  $('home').classList.add('hidden');
  $('edit').classList.remove('hidden');
  $('edTitle').textContent='Редактор — '+p.name;
  refresh();
}
$('createBtn').onclick=function(){ openEditor({name:'Свой фон', st:function(){ return S({layers:[L('grid',{})]}) }}); };
$('back').onclick=function(){ $('edit').classList.add('hidden'); $('home').classList.remove('hidden'); };
...
$('addBtn').onclick=function(){
  state.layers.push(newLayer($('addType').value));
...
function cursorScript(){
  return '<script>(function(){\n'
    +'var s=document.querySelector(".bg svg");if(!s)return;\n'
    +'var nd=[].slice.call(s.querySelectorAll("[data-cfg]")).map(function(g){\n'
    +'  var c=JSON.parse(g.getAttribute("data-cfg"));\n'
    +'  return{c:c,h:s.querySelector("#"+c.cid+"h"),hi:s.querySelector("#"+c.cid+"hi"),ring:s.querySelector("#"+c.cid+"ring"),t:s.querySelector("#"+c.cid+"t"),o:s.querySelector("#"+c.cid+"o")};});\n'
    +'if(!nd.length)return;\n'
    +'addEventListener("pointermove",function(e){\n'
    +'  var m=s.getScreenCTM();if(!m)return;\n'
    +'  var p=s.createSVGPoint();p.x=e.clientX;p.y=e.clientY;p=p.matrixTransform(m.inverse());\n'
...
$('copySVG').onclick=function(){ copy(svgStandalone(),'SVG скопирован ✓'); };
...
$('copyHTML').onclick=function(){ copy(htmlStandalone(),'HTML скопирован ✓'); };
